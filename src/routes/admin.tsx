...
import { IndianRupee, ShoppingBag, Users, Bike, TrendingUp, AlertTriangle } from "lucide-react";
...
export const Route = createFileRoute("/admin")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Monitor orders, revenue, riders and inventory health across FreshKart dark stores." },
      { property: "og:title", content: "Admin dashboard — FreshKart operations" },
      { property: "og:description", content: "Live operations view: revenue, live orders, rider fleet and low-stock alerts." },
...
const riders = [
  { name: "Imran S.", zone: "HSR", trips: 14, rating: 4.9, state: "On trip" },
  { name: "Kavya R.", zone: "Koramangala", trips: 11, rating: 4.8, state: "Idle" },
  { name: "Prakash M.", zone: "BTM", trips: 17, rating: 4.7, state: "On trip" },
  { name: "Nisha T.", zone: "Indiranagar", trips: 9, rating: 5.0, state: "Break" },
];
  ...
  return (
    <ConsoleLayout badge="Admin" title="Operations dashboard" subtitle="Everything happening across your dark stores right now">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-3">
        ...
        <Panel title="Rider fleet">
          <ul className="space-y-3">
            {riders.map((r) => (
              <li key={r.name} className="flex items-center gap-3 rounded-xl border p-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground">{r.zone} · {r.trips} trips · ★ {r.rating}</div>
                </div>
                <Pill label={r.state} tone={r.state === "On trip" ? "primary" : r.state === "Break" ? "offer" : "muted"} />
              </li>
            ))}
          </ul>
        </Panel>
